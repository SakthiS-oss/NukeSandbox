...
| Secret leakage | API key is injected from External Secrets Operator. The raw target URL and hostname are stripped from the Gemini prompt; telemetry mentions are replaced with `<TARGET_URL>` / `<TARGET_HOST>`. | Telemetry can contain sensitive remote content; retention must be controlled in the log platform. |
